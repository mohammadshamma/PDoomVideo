...
//   node render.mjs --frames=0:71.84 --workers=4                                full-res JPEG frames → out/frames (resumable)
//   node render.mjs --encode [--out=out/render_queue.mp4]                               frames + song → MP4
...
const DUR = 74.2, fps = +(args.fps || 24);
const FRAMES_DIR = 'out/frames';

const run = (cmd, a) => new Promise((ok, bad) => { const p = spawn(cmd, a, { stdio: 'inherit' }); p.on('close', c => c ? bad(new Error(cmd + ' exited ' + c)) : ok()); });

if (args.encode) {
  const out = args.out || 'out/render_queue.mp4', n = readdirSync(FRAMES_DIR).filter(f => f.endsWith('.jpg')).length;
  console.log(`encoding ${n} frames → ${out}`);
  await run('ffmpeg', ['-y', '-loglevel', 'error', '-stats', '-framerate', String(fps), '-i', `${FRAMES_DIR}/f%05d.jpg`, '-i', 'assets/I woke up in the render queue.mp3',
    '-map', '0:v', '-map', '1:a', '-af', 'afade=t=out:st=71.5:d=0.34,apad', '-c:v', 'libx264', '-preset', 'slow', '-crf', '17', '-pix_fmt', 'yuv420p', '-c:a', 'aac', '-b:a', '192k',
...
if (args.sheet) {
  ...
  const ff = spawn('ffmpeg', ['-y', '-loglevel', 'error', '-f', 'image2pipe', '-framerate', String(fps), '-c:v', 'mjpeg', '-i', '-',
    '-ss', String(a), '-t', String(b - a), '-i', 'assets/I woke up in the render queue.mp3',
