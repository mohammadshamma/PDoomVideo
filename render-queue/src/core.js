...
const W = 1920, H = 1080;
const BPM = 125, BEAT = 60 / BPM, OFF = 0.32, BOIL = 12, DUR = 74.2;  // the song is 71.84 s; the title card holds ~2.4 s past it
const TAU = Math.PI * 2;
const PAL = {
  ...
  // the render-queue candy palette
  gBlue: '#4F8BF0', gViolet: '#9B72CB', gPink: '#E8738E',            // Gemi's sparkle gradient
  suno: '#2C2538', sunoLt: '#4E4463', wave: '#FFF3DC',              // dark UI plum (karaoke band, bezels)
  sunoA: '#FFA41C', sunoB: '#FF5B24', sunoC: '#FF3D8F',             // Suno's orb gradient: amber → red-orange → pink
  lemon: '#F7D84A', mint: '#79D6AE', coral: '#FF8466', magenta: '#E354A6', cyan: '#5CC8E8', grape: '#5B3F9E', bubble: '#FFB8D2'
};
...
function defineBrushes() {
  ...
  brush.add('fat', { type: 'default', weight: 10, scatter: .05, sharpness: .95, grain: 60, opacity: 255, spacing: .12, pressure: [1, 1], rotate: 'natural', noise: .04 });
...
// The Gemini sparkle outline: four tips joined by concave curves. pinch 0..1 (higher = thinner waist).
function sparklePts(cx, cy, r, pinch = .62, rot = 0, n = 10) {
  const tips = [0, 1, 2, 3].map(i => [cx + Math.cos(rot - Math.PI / 2 + i * Math.PI / 2) * r, cy + Math.sin(rot - Math.PI / 2 + i * Math.PI / 2) * r]);
  return sparkleFromTips(tips, [cx, cy], pinch, n);
}
function sparkleFromTips(tips, c, pinch = .62, n = 10) {
  const p = [];
  for (let i = 0; i < 4; i++) {
    const a = tips[i], b = tips[(i + 1) % 4], m = [(a[0] + b[0]) / 2, (a[1] + b[1]) / 2], k = [lerp(m[0], c[0], pinch), lerp(m[1], c[1], pinch)];
    for (let j = 0; j < n; j++) { const u = j / n; p.push([(1 - u) * (1 - u) * a[0] + 2 * (1 - u) * u * k[0] + u * u * b[0], (1 - u) * (1 - u) * a[1] + 2 * (1 - u) * u * k[1] + u * u * b[1]]); }
  }
  return p;
}
